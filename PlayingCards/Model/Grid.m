...
{
    return [[NSString alloc] initWithFormat:@"width: %f height: %f", size.width, size.height];
}

- (NSString *)description
{
    NSString *description = [NSString stringWithFormat:@"[%@] fitting %lu cells with aspect ratio %g into %@ -> ", NSStringFromClass([self class]), (unsigned long)self.minimumNumberOfCells, self.cellAspectRatio,
                             [self NSStringFromCGSize: self.size]];
    ...
    if (!self.rowCount) {
        ...
        description = [description stringByAppendingFormat:@"%luc x %lur at %@ each", (unsigned long)self.columnCount, (unsigned long)self.rowCount,  [self NSStringFromCGSize: self.cellSize]];
    }
